...
1. MUST: External API calls requiring authentication MUST be proxied through server-side API routes (e.g., /app/api/*/route.ts) rather than called directly from client components

## Policy Block

- MUST External API calls requiring authentication MUST be proxied through server-side API routes (e.g., /app/api/*/route.ts) rather than called directly from client components

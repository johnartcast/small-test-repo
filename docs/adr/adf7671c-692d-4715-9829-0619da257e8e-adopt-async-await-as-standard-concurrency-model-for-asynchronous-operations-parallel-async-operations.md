...
1. SHOULD: Parallel async operations that do not depend on each other SHOULD use Promise.all() or Promise.allSettled() with await

## Policy Block

- SHOULD Parallel async operations that do not depend on each other SHOULD use Promise.all() or Promise.allSettled() with await

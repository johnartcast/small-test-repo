...
1. MUST: API routes MUST implement cache-aside pattern: check cache first, fetch from source on miss, then populate cache

## Policy Block

- MUST API routes MUST implement cache-aside pattern: check cache first, fetch from source on miss, then populate cache

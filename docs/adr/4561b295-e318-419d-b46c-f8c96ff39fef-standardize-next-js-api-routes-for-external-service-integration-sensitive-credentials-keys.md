...
1. MUST: Sensitive credentials (API keys, tokens) MUST be stored in environment variables and accessed only in server-side API routes

## Policy Block

- MUST Sensitive credentials (API keys, tokens) MUST be stored in environment variables and accessed only in server-side API routes

...
1. MUST_NOT: Server-only code (database queries, API keys, sensitive business logic) MUST NOT be placed in client components without proper server boundary markers

## Policy Block

- MUST_NOT Server-only code (database queries, API keys, sensitive business logic) MUST NOT be placed in client components without proper server boundary markers

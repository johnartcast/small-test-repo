# Standardize Next.js API Routes for External API Integration: External Calls Proxied
...
1. MUST: All external API calls MUST be proxied through Next.js API routes (app/api/**/route.ts) or server actions rather than called directly from client components

## Policy Block

- MUST All external API calls MUST be proxied through Next.js API routes (app/api/**/route.ts) or server actions rather than called directly from client components

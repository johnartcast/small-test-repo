...
- **R-EXT-001** MUST: All external API calls MUST be proxied through Next.js API routes (`app/api/**/route.ts`) or server actions rather than called directly from client components.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All three verification commands must return no matches (empty output) for the rule to pass.
</enforcement>
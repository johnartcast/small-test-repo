# Standardize Next.js API Routes for External API Integration: Routes Handle Authentication
...
- **R-AUTH-001** MUST: API routes MUST handle authentication credentials and API keys server-side using environment variables, never exposing them to the client.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All three verification commands must return no matches (empty results) before accepting code changes.
</enforcement>
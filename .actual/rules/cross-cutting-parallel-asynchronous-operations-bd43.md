...
- **R-ASYNC-001** SHOULD: Parallel asynchronous operations SHOULD use Promise.all() or Promise.allSettled() with await rather than sequential await calls when operations are independent.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Violations detected by ESLint or code review must be addressed before merge.
</enforcement>